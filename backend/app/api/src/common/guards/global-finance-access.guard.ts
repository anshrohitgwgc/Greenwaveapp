...
import { assertGreenWaveRecyclingFinanceAccess } from './recycling-finance.guard';
 ...
 * who are authorized for GreenWave Recycling division AND hold global
 * administrative or multi-facility access, preventing single-warehouse-scoped
 * or Healthcare-scoped users from viewing company-wide financials.
 */
@Injectable()
export class GlobalFinanceAccessGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean {
    ...
    // First, strictly verify GreenWave Recycling division eligibility
    assertGreenWaveRecyclingFinanceAccess(user, request);
